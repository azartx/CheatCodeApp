...
fun ConsoleSheetItem(modifier: Modifier, item: CheatSheetItem) {
    Row(modifier = modifier.fillMaxWidth()) {
        Image(painter = painterResource(id = if (item.isFavourite) R.drawable.ic_star_filled else R.drawable.ic_star_blank), contentDescription = "Is Favourite Cheat")
        Column(modifier = Modifier.weight(1f)) {
            Text(text = item.cheat.description)
            Text(text = item.cheat.guide.joinToString())
            Row {
                item.cheat.guide.forEach { command ->
                    Image(modifier = Modifier.size(40.dp), painter = painterResource(id = command.imageResId), contentDescription = "")
                }
            }
        }
    }
